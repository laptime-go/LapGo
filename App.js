    ...
    <View style={s.container}>
      ...
      {tab==='賽道' && (
        <View style={{flex:1}}>
          ...
          {!isPro && <View style={s.ad}><Text style={s.adTxt}>AD {AD_BANNER}</Text></View>}
        </View>
      )}
      {tab==='紀錄' && <ScrollView style={{flex:1, padding:15}}><Text>最佳 {best?fmt(best):'--'}</Text>{laps.map((l,i)=><View key={i} style={s.lapRow}><Text>Lap {i+1}: {fmt(l)}</Text></View>)}</ScrollView>}
      {tab==='設定' && <ScrollView style={{flex:1, padding:15}}><Text>Pro: {isPro?'已買':'未買'} | {AD_BANNER}</Text></ScrollView>}
