...
function bearing(a,b,c,d){const y=Math.sin((d-b)*Math.PI/180)*Math.cos(c*Math.PI/180);const x=Math.cos(a*Math.PI/180)*Math.sin(c*Math.PI/180)-Math.sin(a*Math.PI/180)*Math.cos(c*Math.PI/180)*Math.cos((d-b)*Math.PI/180);return (Math.atan2(y,x)*180/Math.PI+360)%360;}export default function App(){
...
})();return()=>{if(subRef.current)subRef.current.remove();if(timerRef.current)clearInterval(timerRef.current);};},[]);const fmt=s=>{if(!s)return '--:--.--';if(timeFmt==='ss')return `${s.toFixed(2)}s`;const mm=Math.floor(s/60);const r=(s%60).toFixed(2);return `${mm}:${r.padStart(5,'0')}`;};
...
const handleUpgrade=async()=>{const exp=Date.now()+30*24*60*60*1000;setIsPro(true);setProExp(exp);await AsyncStorage.multiSet([['isPro','1'],['pro_exp',String(exp)]]);Alert.alert(lang==='zh'?'已訂閱Pro':'Subscribed','',[{text:'OK',style:'cancel'}],{cancelable:true});};
const DashContent=()=>{
return(
<View>
<View style={[s.btnR,{marginHorizontal:6,marginTop:2,marginBottom:4}]}><TouchableOpacity onPress={stop} style={[s.btn,{backgroundColor:running?'#ff3333':'#551111'}]}><Text style={s.btnT}>■ STOP</Text></TouchableOpacity><TouchableOpacity onPress={start} style={[s.btn,{backgroundColor:running?'#114422':'#00cc66'}]}><Text style={s.btnT}>▶ START</Text></TouchableOpacity></View>
