...
const LiveTimer = memo(({running,startRef,timeFmt,theme})=>{
  const [t,setT]=useState(0);
  const iv=useRef(null);
  useEffect(()=>{
    if(running){
      iv.current=setInterval(()=>{setT((Date.now()-startRef.current)/1000)},100);
    }else{
      if(iv.current){clearInterval(iv.current); iv.current=null;}
    }
    return()=>{if(iv.current)clearInterval(iv.current);};
  },[running]);
  const fmt=s=>{if(!s&&s!==0)return '--:--.--';if(timeFmt==='ss')return `${s.toFixed(2)}s`;const mm=Math.floor(s/60);const r=(s%60).toFixed(2);return `${mm}:${r.padStart(5,'0')}`;};
  return <Text style={[s.big,theme.t]}>{fmt(t)}</Text>;
});
const ControlBar = memo(({running,onStop,onStart})=>{
  return(<View style={s.btnR}><TouchableOpacity onPress={onStop} activeOpacity={0.7} style={[s.btn,{backgroundColor: running? '#ff3333' : '#551111'}]}><Text style={s.btnT}>■ STOP</Text></TouchableOpacity><TouchableOpacity onPress={onStart} activeOpacity={0.7} style={[s.btn,{backgroundColor: running? '#114422' : '#00cc66'}]}><Text style={s.btnT}>▶ START</Text></TouchableOpacity></View>);
...
const [calibDist,setCalibDist]=useState(25);const [minTrigger,setMinTrigger]=useState(15);const [autoLap,setAutoLap]=useState(true);const [unit,setUnit]=useState('kmh');const [tempU,setTempU]=useState('C');const [tempC,setTempC]=useState(null);const [timeFmt,setTimeFmt]=useState('mm:ss');const [showLabels,setShowLabels]=useState(true);const [safeMode,setSafeMode]=useState(true);
const [gpsHz,setGpsHz]=useState(1);const [accFilt,setAccFilt]=useState(10);const [gForce,setGForce]=useState({x:0,max:0});const [resetKey,setResetKey]=useState(0);
const startRef=useRef(0);const subRef=useRef(null);const trackRef=useRef(TRACKS[0]);const runningRef=useRef(false);const maxSpeedLapRef=useRef(0);const finishLapRef=useRef(null);const lastSpeed=useRef(0);const lastManualTrack=useRef(0);const lastPos=useRef(null);const T=LANG[lang];
...
return(
<View style={{flex:1}}>
{running? <LiveTimer key={resetKey} running={running} startRef={startRef} timeFmt={timeFmt} theme={theme}/> : <LiveTimer key={'stopped-'+resetKey} running={false} startRef={startRef} timeFmt={timeFmt} theme={theme}/>}
...
<ControlBar running={running} onStop={stop} onStart={start}/>
...
{!isPro&&<View style={{position:'absolute',bottom:48,left:0,right:0,height:50,backgroundColor:'#000',justifyContent:'center',alignItems:'center',borderTopWidth:1,borderColor:'#222',zIndex:999}}>{BannerAd? <BannerAd unitId={BANNER_ID} size={BannerAdSize.BANNER}/> : <Text style={{color:'#444',fontSize:9}}>AD</Text>}</View>}
...
<Text style={[s.setTitle,theme.t]}>{T.timer}</Text><TouchableOpacity style={[s.setRow,theme.b]} onPress={()=>{const nv=calibDist>=50?10:calibDist+5;setCalibDist(nv);save('calib',nv);}}><Text style={theme.t}>{T.calib} {calibDist}m</Text><Text style={theme.sub}>{calibDist}m ＞</Text></TouchableOpacity><TouchableOpacity style={[s.setRow,theme.b]} onPress={()=>{const nv=minTrigger>=30?5:minTrigger+5;setMinTrigger(nv);save('minTrig',nv);}}><Text style={theme.t}>{T.minTrig} {minTrigger}s</Text><Text style={theme.sub}>{minTrigger}s ＞</Text></TouchableOpacity><View style={[s.setRow,theme.b]}><Text style={theme.t}>{T.autoLap}</Text><Switch value={autoLap} onValueChange={v=>{setAutoLap(v);save('autoLap',v?'1':'0');}}/></View>
<Text style={[s.setTitle,theme.t]}>{T.disp}</Text><View style={[s.setRow,theme.b]}><Text style={theme.t}>{T.night}</Text><Switch value={darkMode} onValueChange={v=>{setDarkMode(v);save('darkMode',v?'1':'0');}}/></View><TouchableOpacity style={[s.setRow,theme.b]} onPress={toggleLang}><Text style={theme.t}>{T.langTitle}</Text><Text style={[theme.t,{fontWeight:'bold',color:'#00cc66'}]}>{lang==='zh'?'中文':'EN'} ↔</Text></TouchableOpacity>
...
<View style={{alignItems:'center',marginTop:14,marginBottom:10}}><Text style={{fontSize:11,color:'#888'}}>v1.0</Text></View>
